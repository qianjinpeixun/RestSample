...
public class A09 {
     ...
     *
     * @param addCounts
     *            the counts for adding to a PQ
     * @param removeCounts
     *            the counts for removing from a PQ
     */
    private static void showResults(List<Integer> addCounts, List<Integer> removeCounts) {
        ...
        System.out.printf("%8s%10s%10s%n", "N", "Adding", "Removing");
        System.out.printf("%8s%10s%10s%n", "------", "------", "--------");
        System.out.printf("%8d%10d%10s%n", 0, addCounts.get(0), "n/a");
        for (int i = 1; i < addCounts.size(); ++i) {
            System.out.printf("%8d%,10d%,10d%n", i, addCounts.get(i), removeCounts.get(i));
        }
        System.out.println();
    }
